...
## 4. Aktualität und kontrollierte Erneuerung der DSL-Skills sicherstellen

### Konkrete Verbesserung

Jeder mit `mps-dsl-memory` erzeugte DSL-Skill erhält nachvollziehbare Herkunfts- und Aktualitätsinformationen:

- Zeitpunkt der letzten Erzeugung beziehungsweise Prüfung
- Sprachname und Sprachmodul-UUID
- verwendetes MPS-Projekt und MPS-Version
- untersuchte Beispielmodelle und stabile Root-Referenzen
- Stand der untersuchten Runtime beziehungsweise Generatorimplementierung
- zuletzt erfolgreich geprüfte Blueprints
- bekannte ungeklärte oder versionsabhängige Aussagen
- manuell ergänzte Hinweise, die bei einer Aktualisierung erhalten bleiben müssen

Eine Aktualitätsprüfung soll mindestens Folgendes kontrollieren:

1. Sprach- und Konzeptreferenzen lassen sich noch auflösen.
2. dokumentierte Properties, Child-Roles und Reference-Roles existieren weiterhin.
3. aufgezeichnete Beispiel-Roots sind noch vorhanden.
4. alle Blueprint-Dateien sind gültiges JSON.
5. ausgewählte Blueprints bestehen einen Dry-Run im vorgesehenen Zielmodell.
6. bekannte Runtime-Aussagen stimmen noch mit Implementierung oder Tests überein.
7. fachliche Dokumentation und Skill widersprechen sich nicht.

Eine Aktualisierung erfolgt inkrementell: automatisch erzeugbare Metadaten werden erneuert, manuelle Architektur- und Runtime-Hinweise bleiben erhalten und werden nur bei nachgewiesener Änderung ersetzt. Neue Erkenntnisse aus MPS- oder Runtime-Analysen lösen einen Drift-Hinweis aus. Das betrifft aktuell insbesondere die Behandlung von `onStartup` und `onShutdown` im DataUX-Skill.

### Offene Fragen

<<Soll die Aktualitätsprüfung automatisch bei jeder relevanten Agentenaufgabe oder nur auf ausdrückliche Anforderung ausgeführt werden?>>:

<<Welche Änderungen sollen zwingend eine Aktualisierung auslösen: Sprachstruktur, Constraints, Generator, Runtime, Beispielmodelle oder alle genannten?>>:

<<Soll ein Skill bei erkanntem Drift weiterhin verwendet werden dürfen, wenn der Agent die betroffene Information live in MPS verifiziert?>>:

<<Wer entscheidet bei einem Widerspruch zwischen fachlicher Dokumentation, DSL-Skill, MPS-Metamodell und Runtimeimplementierung, welche Aussage maßgeblich ist?>>:

<<Sollen Runtime-Quellen und generierter Java-Code regelmäßig in die Aktualitätsprüfung einbezogen werden oder nur bei konkreten Laufzeitfragen?>>:

<<Soll die Aktualisierung automatisch neue Blueprints aus repräsentativen Modellen erzeugen dürfen, oder müssen neue Blueprints einzeln freigegeben werden?>>:

<<Welche manuellen Inhalte müssen bei jeder Skill-Aktualisierung garantiert unverändert erhalten bleiben?>>:

<<Soll für jeden Skill ein eigener Versionsstand geführt werden, oder genügt ein gemeinsamer Stand für ObjectFlow, ManMap und DataUX?>>:

## Vorgeschlagene Umsetzungsreihenfolge

1. Verbindlichkeitsklassen und Quellenpriorität festlegen.
2. Cross-DSL-Vertragsformat festlegen und die wichtigsten Übergänge dokumentieren.
3. DataUX-Matrizen zunächst für MPS-Zulässigkeit, danach für die priorisierten Runtime-Frontends erstellen.
4. Aktualitätsmetadaten und Drift-Prüfung in den `mps-dsl-memory`-Prozess aufnehmen.

Die eigentliche Umsetzung sollte erst beginnen, wenn die offenen Fragen beantwortet sind. Dadurch werden Formate, Geltungsbereich und Quellenprioritäten nicht vom Agenten geraten.
